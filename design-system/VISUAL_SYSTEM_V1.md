...
**Status: COMPLETE — 2026-06-16.** All phases P1–P5 done in code, and the **Figma variables/library foundation** is built (file `bHjVuFiAzzBQdViTAj2Twh` — https://www.figma.com/design/bHjVuFiAzzBQdViTAj2Twh): Primitives collection (30 raw `--c-*` vars), Color collection with **Product/Website modes** (19 semantic tokens aliased to primitives, scopes + `var(--*)` code syntax), Spacing (6) + Radius (2) collections, 14 text styles (product `t1–t6` + website reading scale + mono), and a Foundations doc page proving the two-mode switch. First deliverable unlocked by `UMOJAHUB_DESIGN_FOUNDATION_V1.md` §16. Token reference: `design-system/TOKENS.md`. *(Open follow-on: Figma component variants + Code Connect, which come with the app-shell/website builds.)*
...
4. **P4 — Type + motion + spacing finalization:** scales, subsetting, reduced-motion rule. — ✅ **DONE.** (a) Website reading type scale added (`display`, `display-sm`, `read-h2/h3`, `read-lead/body/meta`) reusing Sora/IBM Plex Sans — same families, reading-density context; + `max-w-reading` (68ch) measure for the Stream. (b) Global `prefers-reduced-motion` rule in `globals.css` (collapses transitions/animations to near-instant; WCAG 2.2 SC 2.3.3 / foundation §10). (c) Motion tokens trimmed to the functional set actually used (durations 150/250 + one standard easing); StoryWorld-era `instant/fast/standard/enter/exit` + `decelerate/accelerate/sharp` removed. (d) Font subsetting: removed unused **Geist + Geist Mono** loads (latin-subset retained; weights already trimmed) — payload cut for the 2G constraint. (e) Spacing: confirmed the 8-pt/4px Tailwind grid as canonical (no change needed). type-check/lint/build/test green.
5. **P5 — Legacy migration + docs:** migrate/remove legacy aliases, publish `TOKENS.md`, Code Connect. — ✅ **DONE.** Migrated ~800 legacy class usages across 60+ files to semantic tokens (1:1 identical hex): `accent-green→brand`, `surface-primary→background`, `surface-elevated→surface`, `surface-secondary→surface-raised`, `text-primary→fg`, `text-secondary→fg-muted`, `text-disabled→fg-disabled` (new `--fg-disabled` token added, both modes). Removed all legacy color tokens from `tailwind.config.ts` and the entire legacy `--color-*/--text-t*/--space-*/--radius-*/motion` var block from `globals.css` (body rule repointed to the canonical font/size). Published `design-system/TOKENS.md`. Deleted the throwaway `/tokens-preview` route. Migration **visually verified** (login + marketplace render identically). Zero legacy references remain. type-check/lint/build/test green. *(Code Connect deferred with the Figma library step.)*
