  ...
  return (
    <div className="space-y-2" role="status" aria-label="Loading list">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="rounded border border-border bg-surface p-4 flex items-center gap-4">
  ...
  return (
    <div className="space-y-1" role="status" aria-label="Loading table">
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div
          key={i}
          className="flex gap-4 px-4 py-3 border-t border-border items-center"
        >
