...
export function FoodHubPage({
  ...
  children,
}: FoodHubPageProps): React.ReactElement {
  return (
    <>
      {/* Dark header */}
      ...
      <div className="xl:hidden px-6 md:px-0">
        <SectionAnchor sections={sections} hub="food" />
      </div>
      ...
      <section className="border-t border-ws-border-light bg-ws-surface-raised">
        <div className="max-w-7xl mx-auto px-6 py-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link
            href={registerHref}
            className="inline-flex items-center justify-center min-h-[44px] px-6 border border-ws-hub-green text-ws-hub-green font-geist text-ws-body hover:bg-ws-hub-green hover:text-white transition-colors duration-150 shrink-0"
          >
            {registerLabel}
          </Link>
        </div>
