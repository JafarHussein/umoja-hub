          ...
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="flex items-center justify-between px-4 py-3 border-b border-zinc-800/50 last:border-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="h-9 w-16 bg-surface-secondary rounded-sm animate-pulse" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="min-h-screen bg-surface-primary">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
          <div className="bg-surface-elevated border border-zinc-800/50 rounded-[4px] p-8 text-center">
            <p className="text-t4 font-body text-text-secondary">Failed to load lecturers.</p>
            <button
    ...
    <div className="min-h-screen bg-surface-primary">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-surface-elevated border border-zinc-800/50 rounded-[4px] px-3 py-2">
              <span className="text-t6 font-mono text-text-disabled">PENDING </span>
              <span className="text-t5 font-mono font-semibold text-amber-400 tabular-nums">
                {unverified.length}
              </span>
            </div>
            <div className="bg-surface-elevated border border-zinc-800/50 rounded-[4px] px-3 py-2">
              <span className="text-t6 font-mono text-text-disabled">VERIFIED </span>
              <span className="text-t5 font-mono font-semibold text-accent-green tabular-nums">
                {verified.length}
              </span>
            </div>
          </div>
        ...
        {lecturers.length === 0 ? (
          <div className="bg-surface-elevated border border-zinc-800/50 rounded-[4px] p-8 text-center">
            <p className="text-t4 font-body text-text-secondary">No lecturer accounts registered yet.</p>
          </div>
        ) : (
          <div className="bg-surface-elevated border border-zinc-800/50 rounded-[4px] overflow-hidden">
              ...
              return (
                ...
                >
                  <div className="space-y-0.5 min-w-0 mr-4">
                    ...
                    <p className="text-t6 font-body text-text-secondary truncate">
                      {lecturer.email}
