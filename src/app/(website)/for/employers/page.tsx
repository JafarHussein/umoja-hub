...
import { DataPanel } from '@/components/website/DataPanel';
...
const SECTIONS: AnchorSection[] = [
  ...
  { id: 'verification-payload', label: 'Verification payload' },
...
const EXISTING_TOOLS = [
  ...
  },
  {
    ...
    fails:
      "A degree certifies attendance and exam performance on structured assessments. It does not certify: the ability to work on an unstructured brief, planning discipline before beginning, documentation quality for a non-academic reader, or the capacity for honest self-assessment.",
  },
] as const;
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer:
      "The verification chain is institution-independent. The reviewer was assigned from the verified reviewer pool, not from the student's institution. A student from a regional university who received VERIFIED from a named, credentials-confirmed lecturer at a different institution has evidence that is portable and does not depend on institutional prestige. That is a deliberate design decision.",
  },
  {
...
const MISCONCEPTIONS = [
  ...
  },
  {
    ...
    correction:
      'The review process assesses whether documentation reflects genuine engagement — reviewers look for evidence of real thinking, documented failure, and specific reasoning. But the system cannot forensically prove authorship. VERIFIED is a process assessment, not an authorship certificate.',
  },
  {
    belief: 'I can use a portfolio entry as a substitute for a technical interview.',
    correction:
      "The portfolio is an interview preparation tool, not a hiring decision. It tells you what the student documented and how a qualified reviewer assessed it. It does not tell you how the student performs under time pressure, how they communicate verbally, or how they function in a team. Read the portfolio before the interview. Use it to direct the interview. Do not use it to skip the interview.",
  },
  {
  ...
  },
  {
    ...
    correction:
      "The reviewer was assigned from the verified reviewer pool without regard to the student's institution. An institution's prestige is not a factor in the rubric. The verification chain is designed to be institution-independent precisely because institutional prestige has historically disadvantaged capable students from smaller universities.",
  },
] as const;
...
export default function EmployersPage(): React.ReactElement {
  return (
    ...
    >
      ...
      <section id="the-problem" className="py-12 border-b border-ws-border-light">
        ...
        <SectionH2>The junior developer hiring pipeline charges you for every screening failure.</SectionH2>
        <div className="space-y-5 max-w-3xl">
          ...
          <Body>
            ...
            interviews to find two candidates worth advancing has spent eight interviews producing no
            useful information.
          </Body>
          <Body>
            ...
            generated code has degraded at exactly the moment when junior developers are most likely
            to have been trained on that signal.
          </Body>
          <Body>
            ...
            The reasoning behind the decisions they made, documented honestly, including what did not
            work. The Education Hub&apos;s portfolio system is a partial solution to this problem. Read
            the limitations section before drawing conclusions from a portfolio entry.
          </Body>
        </div>
      </section>

      {/* ── What existing tools show ── */}
      <section id="what-existing-tools-show" className="py-12 border-b border-ws-border-light">
        <SectionLabel>What existing tools show</SectionLabel>
        <SectionH2>What each portfolio mechanism provides and where it structurally fails.</SectionH2>
        <p className="font-geist text-ws-body-lg text-ws-text-secondary leading-[1.7] mb-8 max-w-2xl">
          These are not criticisms of individual candidates. They are structural features of how each
          mechanism was designed, which determine what it can and cannot prove.
        </p>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-ws-border-light mb-6">
          {EXISTING_TOOLS.map((tool) => (
            <div key={tool.name} className="bg-ws-surface-base p-6">
              ...
              <p className="font-geist text-ws-body text-ws-text-secondary mb-3">
                <span className="text-ws-text-primary italic">Provides: </span>
                {tool.provides}
              </p>
        ...
        <div className="border border-ws-border-light p-5 max-w-3xl">
          <p className="font-geist text-ws-body text-ws-text-secondary">
            <span className="font-semibold text-ws-text-primary">What UmojaHub adds: </span>
      ...
      <section id="the-verification-chain" className="py-12 border-b border-ws-border-light">
        <SectionLabel>The verification chain</SectionLabel>
        <SectionH2>Who reviewed this entry, in what sequence, with what constraints.</SectionH2>
        <p className="font-geist text-ws-body-lg text-ws-text-secondary leading-[1.7] mb-8 max-w-2xl">
          Trust in this system is not asserted. It is constructed from a chain of verified decisions,
          each of which can be traced independently.
        </p>
          ...
          {VERIFICATION_STEPS.map((step) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="font-geist text-ws-body text-ws-text-secondary">{step.detail}</p>
                {step.note !== null && (
      ...
      {/* ── What VERIFIED means ── */}
      <section id="what-verified-means" className="py-12 border-b border-ws-border-light">
        <SectionLabel>What VERIFIED means</SectionLabel>
        <SectionH2>Read this section before making any hiring decision based on a portfolio entry.</SectionH2>
        ...
        <div className="border border-ws-border-light p-5 max-w-3xl">
          <p className="font-geist text-ws-body text-ws-text-secondary">
            <span className="font-semibold text-ws-text-primary">The calibration point: </span>
            ...
            most informative artifact in the portfolio. The reviewer&apos;s commentary is the substantive
            assessment; the badge is the administrative decision.
          </p>
        </div>
      </section>

      {/* ── Verification payload ── */}
      <section id="verification-payload" className="py-12 border-b border-ws-border-light">
        <SectionLabel>Verification payload</SectionLabel>
        <SectionH2>What the backend generates for every VERIFIED decision.</SectionH2>
        <p className="font-geist text-ws-body-lg text-ws-text-secondary leading-[1.7] mb-6 max-w-2xl">
          Every verified portfolio entry is backed by a structured verification record. The sample
          below is the schema generated for a VERIFIED decision on the AI_BRIEF track.
        </p>

        <div className="max-w-2xl mb-4">
          <DataPanel label="VERIFICATION PAYLOAD (sample)">
            <pre className="text-[13px] leading-[1.6] overflow-x-auto whitespace-pre">{`{
  ...
  "reviewerHash":     "sha256:a3f2c1d9e8b706...",
  "decision":         "VERIFIED",
  "track":            "AI_BRIEF",
  "dimensionScores": {
  ...
  "documentHashes": {
    "breakdown":  "sha256:9d1e2f3a4b5c6d7e...",
    "plan":       "sha256:4c5d6e7f8a9b0c1d...",
    "reflection": "sha256:8b9c0d1e2f3a4b5c..."
  },
  "publicRecord": "https://umojahub.com/verify/vrf_01HXTZ4K9QVNBP8M3CJRYL2A6"
          ...
          {[
            {
              field: 'reviewerHash',
              meaning:
                'A cryptographic signature of the reviewer\'s credential record at the time of review. Anchors the review to the specific verified credential state — not just the reviewer\'s name.',
            },
            {
              field: 'documentHashes',
              meaning:
                'SHA-256 hashes of the three submitted documents. A document hash that matches the currently displayed document confirms no alteration after submission.',
            },
            {
              field: 'publicRecord',
              meaning:
                'A stable URL for independent verification. Any third party can access the record without requiring platform authentication.',
            },
          ].map((item) => (
            <div key={item.field} className="flex items-start gap-4">
              <code className="font-geist-mono text-ws-data text-ws-hub-blue shrink-0 mt-0.5">
                {item.field}
              </code>
              <p className="font-geist text-ws-body text-ws-text-secondary">{item.meaning}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="reading-the-reflection" className="py-12 border-b border-ws-border-light">
        <SectionLabel>Reading the Reflection</SectionLabel>
        <SectionH2>The Reflection document is the most useful hiring artifact in the portfolio.</SectionH2>
        <div className="space-y-5 max-w-3xl mb-8">
          <Body>
            Technical interviewers spend significant time trying to understand how a candidate thinks
            about failure. How do they describe what went wrong? Do they take ownership or deflect?
            Can they explain the technical reason for a failure at depth? Do they have a hypothesis
            about what would have worked differently?
          </Body>
          <Body>
            The Reflection document, written before any interview, answers these questions in advance.
            The student documented what they built, what they did not finish, why they did not finish
            it, and what they would do differently. This is qualitatively different from asking
            &quot;what was your greatest challenge?&quot; in a generic interview — the Reflection gives a
            specific challenge already documented before the interview begins.
          </Body>
          <Body>
            Strong Reflection documents have a specific texture: they describe failure with
            specificity, they explain what the student tried before accepting a workaround, and they
            propose what a proper solution would require — even if that solution was beyond the
            student&apos;s capacity at the time. Weak Reflection documents acknowledge that something was
            hard without explaining why or what was learned.
          </Body>
        </div>

        <p className="font-geist text-ws-body-sm text-ws-text-tertiary mb-3">
          Example — what a strong Reflection extract looks like
        </p>
        <div className="bg-ws-surface-raised border border-ws-border-light p-5 max-w-3xl mb-3">
          <p className="font-geist-mono text-ws-caption text-ws-text-tertiary uppercase mb-3">
          ...
          <p className="font-geist text-ws-body text-ws-text-secondary">
            ...
            My initial approach used client-side queueing with timestamps to sequence operations from
            multiple devices. This failed during testing because of clock drift — two devices queueing
            the same observation within seconds could generate conflicting timestamps that the merge
            logic could not resolve deterministically. I worked around this by restricting the data
            model to single-device entry for each field session, which eliminates the conflict but
            also eliminates collaborative data collection. A proper solution would require a
            conflict-resolution algorithm — most likely a variant of vector clocks or CRDTs — that
            I did not have time to implement. I understand the approach now but did not implement
            it. The workaround is in the code; the explanation of what it costs is in this
            section.&quot;
          </p>
        </div>
        <p className="font-geist text-ws-body-sm text-ws-text-tertiary max-w-3xl">
      ...
      <section id="as-an-interview-tool" className="py-12 border-b border-ws-border-light">
        <SectionLabel>As an interview tool</SectionLabel>
        <SectionH2>How to use the portfolio before, during, and after the interview.</SectionH2>
        <p className="font-geist text-ws-body-lg text-ws-text-secondary leading-[1.7] mb-8 max-w-2xl">
          The portfolio is not a hiring decision. It is a preparation tool that makes interviews more
          efficient by directing attention to the most revealing evidence before the conversation
          begins.
        </p>
        ...
        <div className="border-l-2 border-ws-border-medium pl-6 space-y-8 max-w-3xl">
          <div>
            ...
            <Body>
              Identify the two or three specific technical challenges the student documented. Note the
              points where they acknowledged failure, made a workaround, or described what a better
              solution would require. These are your interview questions — already written by the
              candidate before you spoke to them.
            </Body>
          </div>
          ...
          <div>
            ...
            <Body>
              ...
              Reflection describes a synchronization failure, ask: &quot;You mentioned vector clocks as
              the proper solution. Walk me through how that would work.&quot; If they can extend the
              reasoning fluently, the portfolio entry is validated by the interview. If they cannot
              explain what they wrote, that is also useful information — and a more informative
              signal than a generic technical question would have produced.
            </Body>
          </div>
          ...
          <div>
            ...
            <Body>
              A candidate who could fluently extend the reasoning documented in their Reflection gives
              you high confidence that the portfolio reflects genuine work. A candidate who could not
              explain their documented reasoning at depth gives you a different signal. In both cases,
              the portfolio made the interview more efficient than a cold technical screen would have
              been.
            </Body>
          </div>
        </div>
      ...
      <section id="independent-verification" className="py-12 border-b border-ws-border-light">
        <SectionLabel>Independent verification</SectionLabel>
        <SectionH2>How to confirm a portfolio entry is authentic without trusting the platform.</SectionH2>
        ...
        <div className="bg-ws-surface-raised border border-ws-border-light overflow-hidden max-w-3xl mb-6">
          <div className="px-5 py-3 border-b border-ws-border-light">
            <p className="font-geist-mono text-ws-caption text-ws-text-tertiary uppercase">
          ...
          {VERIFY_STEPS.map((item) => (
            <div
              key={item.step}
              className="flex items-start gap-4 px-5 py-4 border-b border-ws-border-light last:border-0"
            >
        ...
        <div className="border border-ws-border-light p-5 max-w-3xl">
          <p className="font-geist text-ws-body text-ws-text-secondary">
            <span className="font-semibold text-ws-text-primary">What a matching hash confirms: </span>
      ...
      <section id="a-real-scenario" className="py-12 border-b border-ws-border-light">
        ...
        <SectionH2>A hiring manager encounters a verified portfolio for the first time.</SectionH2>
        <p className="font-geist text-ws-body-lg text-ws-text-secondary leading-[1.7] mb-8 max-w-2xl">
        ...
        <div className="border-l-2 border-ws-border-medium pl-6 space-y-8 max-w-3xl">
          <div>
            ...
            <Body>
              A hiring manager at a Nairobi fintech is reviewing CVs for a junior developer role. A
              shortlisted candidate has included a UmojaHub portfolio URL alongside their CV and
              GitHub link. The hiring manager has not seen this before. They click the link with low
              expectations.
            </Body>
          </div>
          ...
          <div>
            ...
            <Body>
              The portfolio page loads without requiring registration. They see: a project title (crop
              health observation tool for smallholder farmers), Agriculture track, AI_BRIEF type,
              verification date, a reviewer name and institutional affiliation, a peer score of
              3.8/5.0 across four dimensions, and three expandable documents.
            </Body>
            <div className="mt-3">
              <Body>
          ...
          <div>
            ...
            <Body>
              Could this be fabricated? The hiring manager notes the document hash. They download the
              Reflection document and run a SHA-256 hash. It matches the hash on the page. They search
              the reviewer name — the reviewer is a lecturer in the Department of Computer Science at
              a Kenyan university. They exist. They teach there.
            </Body>
            <div className="mt-3">
              <Body>
          ...
          <div>
            ...
            <Body>
              They ask: &quot;Your Reflection mentions CRDTs as the solution you did not have time to
              implement. Walk me through how a CRDT would address the clock drift problem you
              described.&quot; The student explains, describes merge semantics at depth, acknowledges
              where their understanding ends, and asks a clarifying question about the fintech&apos;s data
          ...
          <div>
            ...
            <Body>
              The hiring manager advanced the candidate. The portfolio entry did not make the hiring
              decision — the interview did. But the portfolio made the interview more efficient by
              providing a specific technical conversation before it began, and by giving the hiring
              manager a way to calibrate whether the documentation reflected genuine understanding.
            </Body>
            <div className="mt-3">
      ...
      <section id="limitations" className="py-12 border-b border-ws-border-light">
        ...
        <SectionH2>What the platform cannot tell you.</SectionH2>
        <p className="font-geist text-ws-body-lg text-ws-text-secondary leading-[1.7] mb-8 max-w-2xl">
      ...
      <section id="faq" className="py-12 border-b border-ws-border-light">
        ...
        <SectionH2>Questions employers ask.</SectionH2>
      ...
      <section id="misconceptions" className="py-12 border-b border-ws-border-light">
        ...
        <SectionH2>Common beliefs that produce incorrect hiring signals.</SectionH2>
      ...
      <section id="next-steps" className="py-12">
        ...
        <SectionH2>If a student has sent you a portfolio link.</SectionH2>
        <div className="space-y-5 max-w-3xl">
          <Body>
            Start with the Reflection document. Read it in full before opening the Breakdown or the
            Plan. The Reflection is the most diagnostic artifact in the portfolio — it reveals
            professional thinking quality more directly than any other piece of evidence you will
            find in a junior developer&apos;s application.
          </Body>
          <Body>
            Note two or three specific technical claims or failures documented in the Reflection.
            Use these as interview questions. Ask the candidate to extend the reasoning they
            documented. The interview will tell you whether the portfolio reflects genuine work.
          </Body>
          <Body>
            If you want to verify the document hash or look up the reviewer&apos;s credentials, the
            Independent verification section of this page explains how.
          </Body>
        </div>
      </section>
    </EduHubPage>
