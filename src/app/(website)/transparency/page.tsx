...
const infrastructure: InfraItem[] = [
  ...
  {
    provider: 'Groq',
    role: 'AI Farm Assistant (farmer-facing agricultural guidance) and AI Mentor (student-facing Socratic questioning during project work)',
    note: null,
  },
  {
    provider: 'OpenAI',
    role: 'AI_BRIEF project brief generation from the agricultural context library, and knowledge article content moderation',
    note: null,
  },
  {
    provider: 'Cloudinary',
...
export default async function TransparencyPage(): Promise<React.ReactElement> {
  ...
  return (
    <AudiencePage
      eyebrow="Platform Transparency · All Audiences"
      heading="What the platform measures, what it does not, and who runs the infrastructure."
      intro="This page publishes UmojaHub's live operational metrics, explains exactly what each metric counts and does not count, and discloses the infrastructure providers and AI systems in use. Updated every five minutes from the platform database."
      sections={sections}
      registerHref="/contact"
      registerLabel="Contact us with a data inquiry"
    >
      ...
      <section id="live-metrics" className="py-12">
        ...
        <p className="font-mono text-t6 text-text-disabled mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-zinc-800/50 mb-8">
          ...
          <div className="bg-surface-primary p-6">
            <p className="font-mono text-t1 font-semibold text-text-primary tabular-nums mb-1">
            ...
            <p className="font-body text-t5 text-text-disabled leading-relaxed">
        ...
        <div className="bg-surface-elevated border border-zinc-800/50 rounded-sm overflow-hidden">
          {[
            ...
            <div key={i} className="px-5 py-5 border-b border-zinc-800/50 last:border-0">
              <p className="font-body text-t5 font-semibold text-text-primary mb-2">{item.metric}</p>
              <p className="font-body text-t5 text-text-secondary leading-relaxed mb-2">{item.why}</p>
      ...
      <section id="what-we-do-not-track" className="py-12">
        ...
        <div className="border border-zinc-800/50 rounded-sm p-5 max-w-3xl mt-6">
      ...
      <section id="infrastructure" className="py-12">
            ...
            <div key={i} className="px-5 py-5 border-b border-zinc-800/50 last:border-0">
              <div className="flex flex-col sm:flex-row sm:items-start sm:gap-6">
                <div className="shrink-0 sm:w-44 mb-2 sm:mb-0">
                  <p className="font-mono text-t5 font-semibold text-text-primary">{item.provider}</p>
                </div>
                <div className="flex-1">
                  ...
                  {item.note !== null && (
                    <p className="font-body text-t5 text-text-disabled leading-relaxed mt-1">
                      {item.note}
                    </p>
                  )}
                </div>
        ...
        <div className="border border-zinc-800/50 rounded-sm p-5 max-w-3xl mt-6">
          ...
          <p className="font-body text-t4 text-text-secondary leading-relaxed mb-3">
            UmojaHub uses AI at two specific points: the AI Farm Assistant and AI Mentor (Groq),
            and brief generation and content moderation (OpenAI). AI is not used for any
            verification decision — farmer, supplier, or lecturer credentials are reviewed by
            human administrators only. AI is not used for any component of the Trust Score
            calculation. AI is not used for any reviewer assignment or lecturer review decision.
          </p>
          <p className="font-body text-t5 text-text-secondary leading-relaxed">
            Where AI is used, its specific limitations are disclosed on the audience pages for
            farmers (AI Farm Assistant) and students (AI Mentor). The constraints are: no memory
            between sessions, general knowledge only, and for the Mentor specifically — it asks
      ...
      <section id="service-status" className="py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-px bg-zinc-800/50 mb-8">
          {[
            { service: 'Marketplace', status: 'Operational' },
            { service: 'M-Pesa payments', status: 'Operational' },
            { service: 'Education Hub', status: 'Operational' },
            { service: 'AI Farm Assistant', status: 'Operational' },
            { service: 'AI Mentor', status: 'Operational' },
            { service: 'Verification queue', status: 'Operational' },
          ].map((item) => (
            <div key={item.service} className="bg-surface-primary p-6 flex items-center justify-between gap-4">
              <p className="font-body text-t5 text-text-secondary">{item.service}</p>
              <span className="font-mono text-t6 text-accent-green uppercase tracking-widest shrink-0">
                {item.status}
              </span>
            </div>
          ))}
