...
| Ten planning, checklist and research documents that each carried portfolio/employer references | **Resolved 2026-08-25 by the documentation cleanup.** Nine were deleted outright — each was a spent checklist, a superseded gate artefact, or a research input already absorbed into the approved foundations. The tenth, `webapp-reset/ONBOARDING_DESIGN_DIRECTION_V1.md`, is retained; its only remaining "credential" reference is the lecturer's faculty credential, which is unaffected by this reset. |
