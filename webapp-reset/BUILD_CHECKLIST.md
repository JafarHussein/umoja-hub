...
- [ ] **Illustrations into named slots (owner: do during hi-fi, 2026-06-18).**
  - [x] **Asset pipeline as code** — `scripts/fetch-illustrations.ts` (`npm run illustrations`): resolves concept/state scenes from unDraw (`GET /api/search?q=`), recolours the accent → brand green (`#2F6F5E`), optimizes with `svgo` (viewBox kept), emits `public/illustrations/manifest.json`. **7 concept/state SVGs shipped** (secure-login, verification, data-security, farming; empty, error, success). Committed `c1f5ed1`. Independent of the undraw MCP (which is still not live — needs Claude Code restart + project-MCP approval).
  - [ ] **4 role characters still need a Humaaans export** (`char-farmer`, `char-buyer`, `char-student`, `char-lecturer`) — Humaaans/Blush are builders, not fetchable. Build per `CHARACTER_SET_BRIEF_V1.md` (Kenyan skin tone/dress, dignified posture), export SVG into `public/illustrations/_raw/<name>.svg`, then re-run `npm run illustrations` to optimize them in. They show `status: "pending"` in the manifest until then.
  - [ ] Place the shipped/optimized SVGs into the grayscale named slots in Figma (`createNodeFromSvg`), recoloured to brand-green/murram.
