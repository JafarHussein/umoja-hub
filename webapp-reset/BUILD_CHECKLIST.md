...
- [x] **Illustrations into named slots (owner: do during hi-fi, 2026-06-18). unDraw-only — Humaaans dropped (owner ruling 2026-06-19).**
  ...
  - [ ] **Remaining 7 SVGs have no named slot in the current hi-fi** (committed as assets, for Phase 6): 4 role characters (Intent uses text `choice/*` cards, Role-confirm is text), `concept-farming` (Farm-setup is text), `state-empty`/`state-error` (System-states uses 72px icon glyphs, not illustration slots). Wire these in code, or add slots if a later design pass wants them.
