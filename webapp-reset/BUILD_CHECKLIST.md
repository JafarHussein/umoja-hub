...
- [ ] **Illustrations into named slots (owner: do during hi-fi, 2026-06-18).** BLOCKED on **undraw MCP not live** — `.mcp.json` committed but server not connected; needs Claude Code restart + project-MCP approval, then `/mcp` shows `undraw`. Pipeline: `search_svgs` → `get_svg` → `createNodeFromSvg`, recolored to brand-green/murram, into the grayscale slots.
